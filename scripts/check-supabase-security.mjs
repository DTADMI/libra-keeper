...
for (const file of files) {
  ...
  for (const block of blocks) {
    ...
    const inPublicSchema = !functionName.includes(".") || functionName.startsWith("public.");
    ...
    const grantPattern = new RegExp(
      `grant\\s+execute\\s+on\\s+function\\s+public\\.${bareFunctionName}\\s*\\([^\\)]*\\)\\s+to\\s+service_role`,
      "i",
    );
    ...
    if (!grantPattern.test(content)) {
      fail(`${file}: SECURITY DEFINER function ${functionName} must grant execute to service_role in the same migration.`);
    }
  }
}
