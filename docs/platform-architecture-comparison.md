...
const { data: relationships } = await supabase
  ...
  .select("*, custom_relationship_types(*)")
  .eq("user_id", id);
...
const { data: item } = await supabase.from("items").select("*, tags(*)").eq("id", id).single();
...
export const getById = query({
  ...
  handler: async (ctx, args) => {
    ...
    const loans = await ctx.db
      .query("loans")
      .withIndex("by_item", (q) => q.eq("itemId", args.itemId))
      .filter((q) => q.or(q.eq("status", "PENDING"), q.eq("status", "APPROVED")))
      .collect();
...
_This document was generated from comparative analysis of Supabase, Convex, SpacetimeDB, and Neo4j as applied to the QuestHunt, StoryForge, LibraKeeper, and VelvetGalaxy projects within Nebula Forge Digital Studio. Last updated 2026-05-15._
