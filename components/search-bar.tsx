    ...
    <div className="relative w-full max-w-md" ref={ref}>
      ...
      {open && (
        <div className="absolute z-50 mt-2 w-full rounded-md border bg-background shadow-lg">
          {isLoading && (
            <p className="p-4 text-sm text-muted-foreground">{t("Search.searching")}</p>
          )}
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {item.author && (
                    <span className="text-xs text-muted-foreground">{t("Search.by", { author: item.author })}</span>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                {item._count.likes > 0 && (
                  <Badge variant="secondary" className="text-xs">
                    {t("Search.likes", { count: item._count.likes })}
                  </Badge>
                )}
              </div>
            </Link>
